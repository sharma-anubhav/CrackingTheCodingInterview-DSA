...
A set of n items, where each item has:
    A weight w[i]
    A value v[i]
Goal: Select items to put in the knapsack such that:
    The total weight does not exceed W
    The total value is maximized
    Each item can be selected at most once (you cannot take fractional amounts)
"""

"""
LET US UNDERSTAND WHAT EACH TYPE OF RECURSION MEANS.
IT IS IMPORTANT TO KNOW WhAT YOURE RECURSIVE FUNCITON REPRESENTS BEFORE YOU JUMP INTO DP
"""
...
"""Get profit tells us: 
Given cur_weight, i elements processed, what is the max current profit. 

- States tracked: Current weight, current index, max current profit
...
def knapsack(w, v, W):
    ...
    def get_profit(cur_weight, i, cur_profit):
        ...
        yes = -float("inf")
        if cur_weight + w[i] <= W:
            yes = get_profit(cur_weight+w[i], i+1, cur_profit+v[i])
...
def knapsack(w, v, W):
    ...
    def get_profit(cur_weight, i):
        ...
        yes = -float("inf")
        if cur_weight + w[i] <= W:
            yes = v[i]+ get_profit(cur_weight+w[i], i+1)
